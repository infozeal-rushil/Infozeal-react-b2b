...
import { useCalendarContext } from '@globals/g-providers/CalendarProvider';
import { ButtonGroup, Col, Row } from 'react-bootstrap';
import { SET_CALENDAR_STATE } from '@globals/g-reducers/CalendarReducer';
const CalendarHeader = () => {
    ...
    const handleCalendarUpdate = (actionType) => {
        if (calendarApi) {
            actionType === 'next'
                ? calendarApi.next()
                : actionType === 'prev'
    ...
    return (<div className="mx-n4 px-4 mx-lg-n6 px-lg-6 border-y border-translucent">
      <Row className="py-3 gy-3 gx-0 justify-content-between">
        <Col xs={6} md="auto" className="order-1 d-flex align-items-center">
          <Button onClick={() => handleCalendarUpdate('today')} variant="phoenix-primary" size="sm" className="px-4">
            Today
          </Button>
        </Col>
        <Col xs={12} md="auto" className="order-md-1 d-flex align-items-center justify-content-center">
          ...
          {calendarApi && (<h3 className="px-3 text-body-emphasis fw-semibold mb-0">
              {title || calendarApi.view.title}
            </h3>)}
        ...
        <Col xs={6} md="auto" className="order-1 d-flex justify-content-end">
          <ButtonGroup size="sm">
            <Button onClick={() => handleCalendarView('dayGridMonth')} variant="phoenix-secondary" className={classNames({
            active: view === 'dayGridMonth'
        })}>
              Month
            </Button>
            <Button onClick={() => handleCalendarView('timeGridWeek')} variant="phoenix-secondary" className={classNames({
