import { faArrowRightToBracket, faEllipsisH } from '@fortawesome/free-solid-svg-icons';
...
const TopNav = () => {
    ...
    return (<div className="bg-primary-subtle py-2">
      <div className="container-medium d-flex align-items-center justify-content-between">
        <Button href="#!" variant="link" className="text-body p-0">
          <FontAwesomeIcon icon={faArrowRightToBracket} className="me-2" transform="down-1"/>
          Agent Login
        </Button>
        <Dropdown>
          <Dropdown.Toggle size="sm" variant="" className="p-0 d-md-none fs-8 dropdown-caret-none">
        ...
        <ul className="d-none d-md-flex gap-5 list-unstyled mb-0">
          {navItems.map((item, index) => (<li key={index}>
              <Link to={item.link} className="lh-1 text-body-tertiary fw-semibold fs-9">
                {item.icon && (<FontAwesomeIcon icon={item.icon} transform={item.transform || undefined} className="me-2"/>)}
                {item.title}
              </Link>
            </li>))}
        </ul>
