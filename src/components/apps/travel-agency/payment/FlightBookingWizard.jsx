import { faCheck, faMoneyBill, faPlane, faUser } from '@fortawesome/free-solid-svg-icons';
...
const FlightBookingWizard = ({ activeItem }) => {
    const doneItems = wizardItems.filter(item => wizardItems.indexOf(item) <
        wizardItems.findIndex(i => i.name === activeItem));
    return (<div className="theme-wizard flight-booking-wizard" style={{ width: '18.125rem' }}>
      <Nav className="justify-content-between nav-wizard nav-wizard-success">
        {wizardItems.map((item, index) => {
            ...
            return (<Nav.Item key={index}>
              <Nav.Link as={Link} to={item.url} className={`fw-semibold ${stepClass}`}>
