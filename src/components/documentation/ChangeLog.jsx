...
import changelogs from '@globals/g-data/changelog';
...
const ChangeLog = () => {
    return (<div>
      ...
      {changelogs.map(changelog => (<PhoenixDocCard className="mb-4" key={changelog.title}>
          <PhoenixDocCard.Header noPreview>
            <div className="d-flex justify-content-between align-items-center">
              ...
              <Button as={Link} variant="link" to={`/migrations/#${changelog.version}`} endIcon={<FontAwesomeIcon icon={faChevronRight} className="fs-10"/>}>
                See migration
              </Button>
            </div>
          </PhoenixDocCard.Header>
          <PhoenixDocCard.Body>
            {changelog.alertText && (<Alert variant={'subtle-warning'} className="d-flex align-items-center">
                <FontAwesomeIcon icon={faCircleInfo} className="text-warning fs-5 me-3"/>
                {changelog.alertText}{' '}
                {changelog.alertLink && (<span>
            ...
            {changelog.logs.new && (<>
                ...
                <ul>
                  {changelog.logs.new.map(file => (<li dangerouslySetInnerHTML={{ __html: file }} key={file}/>))}
                </ul>
              </>)}
            ...
            {changelog.logs.update && (<>
                ...
                <ul>
                  {changelog.logs.update.map(file => (<li dangerouslySetInnerHTML={{ __html: file }} key={file}/>))}
                </ul>
              </>)}
            {/* Fix */}
            {changelog.logs.fix && (<>
                ...
                <ul>
                  {changelog.logs.fix.map(file => (<li dangerouslySetInnerHTML={{ __html: file }} key={file}/>))}
                </ul>
              </>)}
          </PhoenixDocCard.Body>
        </PhoenixDocCard>))}
