...
const BodyColorDemo = () => {
    return (<>
      <tr>
        ...
        <td className="ps-0">
          <div className="p-5 rounded-2" style={{
...
const ColorDemo = ({ color }) => {
    return (<>
      <tr>
        ...
        <td>
          {color === 'body-highlight' ? (<code>text-{color}</code>) : (<code>text-body-{color}</code>)}
        </td>
      </tr>
      <tr>
        <td>
        ...
        <td>
          {color === 'body-highlight' ? (<code>bg-{color}</code>) : (<code>bg-body-{color}</code>)}
        </td>
      </tr>
    </>);
};
const ColorShadesDemo = ({ color }) => {
...
const Color = () => {
    return (<div>
      <DocPageHeader title="Color" description="Phoenix is supported by an extensive color system that themes our styles and components. This enables more comprehensive customization and extension for any project."/>
      <PhoenixDocCard className="mb-4">
        ...
        <PhoenixDocCard.Body>
          <Table responsive className="table-swatches">
            <thead>
              <tr>
                <th className="ps-0" style={{ width: '20%', minWidth: '100px' }}>
                  Name
                </th>
                <th className="ps-0" style={{ width: '20%', minWidth: '130px' }}>
                  Swatch
                </th>
                <th className="ps-0" style={{ width: '40%', minWidth: '330px' }}>
                  CSS variables
                </th>
                <th className="ps-0" style={{ width: '20%', minWidth: '230px' }}>
                  Class
                </th>
              </tr>
            </thead>
            <tbody>
              <BodyColorDemo />
              {['secondary', 'tertiary', 'quaternary', 'body-highlight'].map((color, index) => (<ColorDemo color={color} key={index}/>))}
              {[
            'primary',
            'secondary',
            'success',
            ...
            'warning',
            'info'
        ].map((color, index) => (<ColorShadesDemo color={color} key={index}/>))}
            </tbody>
          </Table>
        </PhoenixDocCard.Body>
      </PhoenixDocCard>
