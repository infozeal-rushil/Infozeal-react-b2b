...
import DocPagesLayout from '@globals/g-layouts/DocPagesLayout';
...
const DraggableExample = () => {
    return (<div className="mb-9">
      <DocPageHeader title="Draggable" description={`${import.meta.env.VITE_TITLE}-React utilizes @dnd-kit/core and @dnd-kit/sortable for its draggable component. @dnd-kit/core and @dnd-kit/sortable is designed to create visually appealing drag-and-drop functionality for lists, making it accessible to all.`} link={{
...
const SortableItem = ({ task, columnId, children }) => {
    ...
    return (<div ref={setNodeRef} style={styles} {...listeners}>
      {children}
    </div>);
};
const Example = () => {
    ...
    const sensor = useGetDndSensor();
    const findColumn = (id) => {
        return columns.find(col => col.tasks.some(task => task.id === id) || col.id === id);
    };
    const handleDragStart = (event) => {
        var _a;
        setActiveTask((_a = event.active.data.current) === null || _a === void 0 ? void 0 : _a.item);
    };
    const handleDragOver = (event) => {
        ...
        if (!activeColumn || !overColumn || activeColumn.id === overColumn.id)
            return;
        setColumns(prevData => {
            const activeTaskIndex = activeColumn.tasks.findIndex(item => item.id === activeId);
            if (activeTaskIndex === -1)
                return prevData;
            const activeItem = activeColumn.tasks[activeTaskIndex];
            const updatedActiveContents = activeColumn.tasks.filter(task => task.id !== activeId);
            const updatedOverContents = [...overColumn.tasks];
            const overItemIndex = overColumn.tasks.findIndex(task => task.id === overId);
            updatedOverContents.splice(overItemIndex >= 0 ? overItemIndex : updatedOverContents.length, 0, activeItem);
            return prevData.map(column => {
                if (column.id === activeColumn.id) {
                    return Object.assign(Object.assign({}, column), { tasks: updatedActiveContents });
                }
                if (column.id === overColumn.id) {
                    return Object.assign(Object.assign({}, column), { tasks: updatedOverContents });
                }
                return column;
            });
        });
    ...
    const handleDragEnd = (event) => {
        ...
        if (!active || !over)
            return;
        const activeColumnId = (_a = active.data.current) === null || _a === void 0 ? void 0 : _a.columnId;
        const overColumnId = ((_b = over.data.current) === null || _b === void 0 ? void 0 : _b.columnId) || over.id;
        if (activeColumnId === overColumnId) {
            const columnIndex = columns.findIndex(col => col.id === activeColumnId);
            const column = columns[columnIndex];
            const oldIndex = column.tasks.findIndex(task => task.id === active.id);
            ...
            const updatedContents = arrayMove(column.tasks, oldIndex, newIndex);
            const newColumns = [...columns];
            newColumns[columnIndex] = Object.assign(Object.assign({}, column), { tasks: updatedContents });
            setColumns(newColumns);
        }
        else {
            ...
            const activeTask = columns[sourceColumnIndex].tasks.find(task => task.id === active.id);
            const newColumns = [...columns];
            newColumns[sourceColumnIndex].tasks = newColumns[sourceColumnIndex].tasks.filter(task => task.id !== active.id);
            if (activeTask && destinationColumnIndex) {
                newColumns[destinationColumnIndex].tasks = [
                    ...newColumns[destinationColumnIndex].tasks,
                    activeTask
    ...
    return (<DndContext sensors={sensor} collisionDetection={closestCorners} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd}>
      <Row>
        {columns.map(list => (<Col lg={6} key={list.id}>
            <SortableContext items={list.tasks.map(item => item.id)} strategy={verticalListSortingStrategy}>
              {list.tasks.map(task => (<SortableItem key={task.id} task={task} columnId={list.id}>
                  <Card className="mb-3">
                    <Card.Body>
                      <p className="mb-0 fw-medium">{task.title}</p>
                    </Card.Body>
      ...
      <DragOverlay>
        {activeTask && (<Card className="mb-3" style={{
                cursor: 'grabbing'
            }}>
            <Card.Body>
              <p className="mb-0 fw-medium">{activeTask === null || activeTask === void 0 ? void 0 : activeTask.title}</p>
            </Card.Body>
          </Card>)}
      </DragOverlay>
    </DndContext>);
