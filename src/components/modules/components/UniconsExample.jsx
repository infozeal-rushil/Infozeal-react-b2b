...
import { uniconList } from '@globals/g-data/icons/uniconList';
import Unicon from '@globals/g-components/base/Unicon';
...
const UniconsExample = () => {
    return (<div className="mb-9">
      <DocPageHeader title="React Unicons" description="Pixel-perfect vector icons as React Components." link={{
      ...
      <DocPagesLayout>
        ...
        <PhoenixDocCard className="mb-4">
          ...
          <PhoenixDocCard.Body code={ExampleCode} scope={{ Unicons, Unicon, UilPlaneDeparture, UilRocket }}/>
        </PhoenixDocCard>
        ...
        <PhoenixDocCard className="mb-4">
          ...
          <Card.Body>
            <Row className="mt-3">
              {Object.keys(uniconList).map(icon => (<Col xs={12} sm={6} md={4} lg={3} key={icon}>
                  <IconCard icon={uniconList[icon]} iconFamily="unicons" name={icon}/>
                </Col>))}
            </Row>
          </Card.Body>
        </PhoenixDocCard>
