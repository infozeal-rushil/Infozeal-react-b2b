...
import RevealDropdown, { RevealDropdownTrigger } from '@globals/g-components/base/RevealDropdown';
...
${basicImportString}
import Badge, { BadgeBg } from '@globals/g-components/base/Badge';
import FilterTab, { FilterTabItem } from '@globals/g-components/common/FilterTab';
import { useMemo } from 'react';
...
const columns = [
    ...
    {
        accessorKey: 'email',
        cell: ({ row: { original } }) => (<Link to={`mailto:${original.email}`}>{original.email}</Link>)
    },
    {
        accessorKey: 'age'
    },
...
const SearchExample = () => {
    ...
    return (<AdvanceTableProvider {...table}>
      <SearchBox placeholder="Search..." size="sm" onChange={handleSearchInputChange} className="mx-auto mb-4"/>
      <AdvanceTable tableProps={{
            size: 'sm',
            className: 'phoenix-table fs-9 mb-0 border-top border-translucent'
        }} rowClassName="hover-actions-trigger btn-reveal-trigger position-static"/>
...
export const projectListTableColumns = [
    {
        ...
        cell: ({ row: { original } }) => {
            const { name } = original;
            return (<Link to="#!" className="text-decoration-none fw-bold fs-8">
          {name}
        </Link>);
        },
        meta: {
    ...
    },
    {
        ...
        cell: ({ row: { original } }) => {
            const { status } = original;
            return (<Badge variant="phoenix" bg={status.type}>
          {status.label}
        </Badge>);
        },
        meta: {
...
const FilterByColumnExample = () => {
    ...
    const handleFilterItemClick = (columnId, value) => {
        const column = getColumn(columnId);
        column === null || column === void 0 ? void 0 : column.setFilterValue(value === 'all' ? '' : value);
    };
    const tabItems = useMemo(() => {
        const getDataCount = (label) => getPrePaginationRowModel().rows.filter(({ original: { status } }) => status.label === label).length;
        return [
            {
                label: 'All',
                value: 'all',
...
const AdvanceTableExample = () => {
    return (<div>
      <DocPageHeader title="Advance Tables" link={{
        ...
        <PhoenixDocCard.Body>
          ...
          <div>
            ...
            <div>
              ...
              <a href="https://tanstack.com/table/v8/docs/api/core/column-def" target="_blank" rel="noreferrer">
        ...
        <PhoenixDocCard className="mb-4">
          ...
          <PhoenixDocCard.Body>
            <p>
              ...
              <a rel="noreferrer" target="_blank" href="https://tanstack.com/table/latest/docs/guide/pagination#manual-server-side-pagination">
                serverside-pagination
              </a>{' '}
              <a rel="noreferrer" target="_blank" href="https://tanstack.com/table/latest/docs/framework/react/examples/pagination-controlled">
                pagination-controlled
              </a>{' '}
              <a rel="noreferrer" target="_blank" href="https://tanstack.com/table/latest/docs/guide/column-filtering#manual-server-side-filtering">
                filtering
              </a>{' '}
              <a rel="noreferrer" target="_blank" href="https://tanstack.com/table/latest/docs/guide/sorting#manual-server-side-sorting">
                sorting
              </a>
              .
            </p>
