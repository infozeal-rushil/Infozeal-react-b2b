...
const CrmStatCard = ({ icon, color, label, title, value, badgeLabel, footerText, badgeBg }) => {
    return (<Card className="h-100">
      <Card.Body>
        <div className="d-flex d-sm-block justify-content-between">
          <div className="border-bottom-sm mb-sm-4">
            <div className="d-flex align-items-center">
              <div className={`d-flex align-items-center icon-wrapper-sm shadow-${color}-100`} style={{ transform: 'rotate(-7.45deg)' }}>
                <FontAwesomeIcon icon={icon} className={`text-${color} fs-7 z-1 ms-2`}/>
              </div>
