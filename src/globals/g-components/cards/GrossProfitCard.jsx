...
import { profitData } from 'data/travel-agency/travelAgency';
const GrossProfitTable = ({ profit, index }) => {
...
const GrossProfitCard = () => {
    return (<Card className="h-xxl-100">
      ...
      <Card.Body>
        <Row className="align-items-center h-100 gy-5">
          <Col xs={12} md={{ span: 'auto', order: 1 }} xl={{ span: 12, order: 0 }} xxl={{ span: 'auto', order: 1 }} className="px-md-8 px-xl-6">
            {/* echart gross profit */}
          ...
          <Col xx={12} md="auto" xl={12} xxl="auto" className="flex-1 h-md-100">
            <div className="d-flex flex-column justify-content-between h-md-100 h-xl-auto h-xxl-100">
              {profitData.map((data, index) => (<GrossProfitTable profit={data} key={index} index={index}/>))}
            </div>
          </Col>
        </Row>
      </Card.Body>
