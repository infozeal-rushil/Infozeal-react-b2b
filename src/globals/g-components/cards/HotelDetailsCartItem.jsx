...
import { faBaby, faBed, faCircleXmark, faMoon, faUser } from '@fortawesome/free-solid-svg-icons';
import { currencyFormat } from 'helpers/utils';
const HotelDetailsCartItem = ({ cartItem, index, className, crossButtonClickHandler, showHotelInfo }) => {
    return (<Card className={className}>
      <Card.Body>
        {!showHotelInfo && (<Button className="p-0 position-absolute end-0 fs-8 mt-n5 me-n2 text-body-tertiary" onClick={() => crossButtonClickHandler(cartItem.id)}>
        ...
        <div className="d-flex flex-wrap gap-2">
          <Badge variant="phoenix" bg="secondary" className="py-1 border-0 text-capitalize">
          ...
          {cartItem.child && (<Badge variant="phoenix" bg="secondary" className="py-1 border-0 text-capitalize">
              <FontAwesomeIcon icon={faBaby} className="fs-9 me-2"/>
              {cartItem.child} Childs
            </Badge>)}
          {cartItem.nights && (<Badge variant="phoenix" bg="secondary" className="py-1 border-0 text-capitalize">
