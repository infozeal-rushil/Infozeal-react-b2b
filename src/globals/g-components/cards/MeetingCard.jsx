      ...
      <Card.Body>
        <div className="d-flex justify-content-between align-items-start flex-wrap mb-4 gap-2">
          <div className="mb-3 mb-sm-0">
            ...
            <div>
              <UilCalendarAlt className="text-primary me-2" width={16} height={16}/>
          ...
          <Avatar.Group total={meeting.assigness.length} size="s">
            {meeting.assigness.slice(0, 4).map(assigne => (<Avatar key={assigne} size="s" src={assigne}/>))}
          </Avatar.Group>
        </div>
        <div className="d-flex align-items-center">
          <Badge variant={meeting.badge.variant} bg={meeting.badge.bg} className="me-2">
            {meeting.badge.text}
          </Badge>
          <div className="d-flex align-items-center flex-1">
            <FontAwesomeIcon icon={faCircle} transform="shrink-6 up-1" className={`text-${meeting.priority.color} me-1`}/>
            <span className="fw-bold fs-9 text-body">
              {meeting.priority.label}
            </span>
          </div>
          <Button variant="phoenix-primary" startIcon={<FontAwesomeIcon icon={faVideo} className="me-2 d-none d-sm-inline-block"/>}>
            Join
          </Button>
        </div>
      </Card.Body>
