...
const TripSummaryCard = ({ selectedTrip }) => {
    return (<Card>
      <Card.Body>
        <img src={selectedTrip.image} alt="" className="w-100 object-fit-cover rounded-2" height={220}/>
        <h3 className="mb-4 mt-5">{selectedTrip.title}</h3>
        <h5 className="fw-normal mb-3">
          <FontAwesomeIcon icon={faCalendar} className="text-body-quaternary me-2"/>
          {selectedTrip.date}
        </h5>
        <h5 className="fw-normal mb-3">
          <FontAwesomeIcon icon={faUser} className="text-body-quaternary me-2"/>
          {selectedTrip.people}
        </h5>
        <h5 className="fw-normal mb-3">
          <FontAwesomeIcon icon={faClock} className="text-body-quaternary me-2"/>
        ...
        <div className="p-3 rounded-2 bg-body-highlight">
          ...
          <div className="d-flex flex-between-center">
            ...
            <h4 className="mb-0">
              USD{' '}
              {numberFormat(selectedTrip.bookingFee + selectedTrip.subTotal, 'standard', {
            minimumFractionDigits: 2
        })}
            </h4>
          </div>
