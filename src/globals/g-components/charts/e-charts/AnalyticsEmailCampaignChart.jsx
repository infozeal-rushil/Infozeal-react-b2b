...
const tooltipFormatter = (params) => {
    ...
    <h6 class="text-body-tertiary"><span class="d-inline-block rounded-circle me-2" style="height: 0.6rem; width: 0.6rem; background:${el.color}"></span>
        ${el.axisValue} : ${typeof el.value === 'object' ? el.value[1] : el.value}
      </h6>
    </div>`;
    return `<div>
...
const AnalyticsEmailCampaignChart = ({ className }) => {
    const chartRef = useRef(null);
    const updateDimensions = () => {
        var _a, _b, _c, _d;
        if (window.innerWidth < 576) {
            (_a = chartRef.current) === null || _a === void 0 ? void 0 : _a.getEchartsInstance().setOption({
                series: [
                    {
                        barWidth: '48px'
                    }
    ...
    return (<ReactEChartsCore ref={chartRef} echarts={echarts} option={getDefaultOptions(getThemeColor, isDark)} className={className}/>);
};
