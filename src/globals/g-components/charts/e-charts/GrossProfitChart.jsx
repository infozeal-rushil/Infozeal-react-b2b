...
data, colors, isRTL, isDark) => ({
    ...
    tooltip: {
        ...
        formatter: (params) => rtlTooltipFormatter(params, isRTL, isDark)
    },
    series: [
        {
            type: 'sunburst',
...
const GrossProfitChart = ({ style }) => {
    const { getThemeColor, config: { isDark, isRTL } } = useAppContext();
    const data = [
        {
            name: 'Flight',
            value: 30,
    ...
    return (<ReactEChartsCore echarts={echarts} option={getDefaultOptions(getThemeColor, data, colors, isRTL, isDark)} style={style} className="mx-auto mt-3 mt-md-0 mt-xl-3 mt-xxl-0"/>);
};
