...
const Comment = ({ comment, className }) => {
    return (<Row className={classNames(className, 'justify-contnet-between g-0 py-3 align-items-start')}>
      <Col xs={12} sm>
        <p className={classNames('fs-9 text-body-secondary mb-0', {
            'mb-2': comment.attachment
        })}>
          <Link to={comment.user.url} className={classNames('fw-semibold', {
        ...
        {comment.attachment && (<img src={comment.attachment} alt="" width={220} className="rounded-2 mb-2"/>)}
      </Col>
