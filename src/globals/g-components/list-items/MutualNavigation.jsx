...
const MutualNavigation = () => {
    return (<>
      ...
      <Row className="g-0 mb-5 mb-lg-0">
        {mutualData.map(item => (<Col xs={12} key={item.label} className="border-bottom border-translucent py-2">
            <Button variant="link" className="ps-0 fs-8 text-body-secondary hover-primary fw-semibold" startIcon={<FontAwesomeIcon icon={item.icon} className="me-2"/>}>
              {item.label}
            </Button>
          </Col>))}
      </Row>
