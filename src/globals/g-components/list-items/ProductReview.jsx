...
const ProductReview = ({ review }) => {
    ...
    return (<RevealDropdownTrigger className="mb-4 hover-actions-trigger">
      ...
      <p className={classNames('text-body-highlight', {
            'mb-3': review.images,
            'mb-1': !review.images
        })}>
        {review.review}
      ...
      {review.images && (<div className="d-flex gap-2 flex-wrap mb-2">
          {review.images.map((image, index) => (<Link to="#!" key={image}>
              <img src={image} key={image} alt="" className="fit-cover w-100" height={164} onClick={() => openLightbox(index + 1)}/>
            </Link>))}
        </div>)}

      {review.reply && (<div className="d-flex">
          <FontAwesomeIcon icon={faReply} className="me-2" transform="rotate-180"/>
          <div>
            <h5>
