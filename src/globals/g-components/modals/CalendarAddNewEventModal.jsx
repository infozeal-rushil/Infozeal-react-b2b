...
const CalendarAddNewEventModal = () => {
    const { openNewEventModal, selectedStartDate, selectedEndDate, calendarDispatch } = useCalendarContext();
    const { formData, setFormData, onChange } = usePhoenixForm({
    ...
    useEffect(() => {
        setFormData(Object.assign(Object.assign({}, formData), { start: selectedStartDate, end: selectedEndDate }));
    }, [selectedEndDate, selectedStartDate]);
    return (<Modal show={openNewEventModal} contentClassName="border-translucent" onHide={handleClose}>
      <Form onSubmit={handleSubmit}>
        ...
        <Modal.Body className="p-card py-0">
          <FloatingLabel controlId="event-title" label="Title" className="mb-3">
            <input className="form-control" onChange={onChange} name="title" type="text" placeholder="Title"/>
          </FloatingLabel>
          ...
          <DatePicker options={{
            ...
            setFormData(Object.assign(Object.assign({}, formData), { start: date }));
        }} render={(_, ref) => {
            return (<Form.Floating className="mb-3">
                  <Form.Control type="text" placeholder="Starts at" ref={ref} id="startDatepicker"/>
                  <label htmlFor="startDatepicker" className="ps-6">
                    Starts at
                  </label>
                </Form.Floating>);
        }}/>
          <DatePicker value={selectedEndDate} options={{ enableTime: true, dateFormat: 'Y-m-d H:i' }} onChange={([date]) => {
            setFormData(Object.assign(Object.assign({}, formData), { end: date }));
        }} render={(_, ref) => {
            return (<Form.Floating className="mb-3">
                  <Form.Control type="text" placeholder="Ends at" ref={ref} id="endDatepicker"/>
          ...
          <Form.Check>
            <Form.Check.Input onChange={onChange} name="allDay" type="checkbox" id="all-day" checked={formData.allDay === true}/>
            <Form.Check.Label htmlFor="all-day">All day event</Form.Check.Label>
          </Form.Check>
          <div className="py-5">
            <FloatingLabel controlId="description" label="Description">
              <textarea className="form-control" onChange={onChange} placeholder="Description" name="description" style={{ height: '128px' }}/>
            </FloatingLabel>
          </div>
          <FloatingLabel controlId="repetition" label="Repetition" className="mb-3">
          ...
          <Button variant="link" className="p-0 mb-3" startIcon={<FontAwesomeIcon icon={faPlus} className="me-2"/>}>
            Add Reminder
          </Button>
        </Modal.Body>
        <Modal.Footer className="d-flex justify-content-between align-items-center border-0">
          <Link to="/apps/events/create-an-event" className="me-3 fs-9 text-body">
            More options
