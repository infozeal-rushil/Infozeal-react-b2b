...
const DealsAddStageModal = ({ show, handleClose }) => {
    const { onChange, formData, setFormData } = usePhoenixForm();
    ...
    return (<Modal show={show} onHide={handleClose} centered contentClassName="border">
      <Form onSubmit={handleSubmit}>
        <Modal.Header className="border-0 p-5">
          <h5 className="modal-title text-body-highlight fs-6 lh-sm">
            Create New Stage
          </h5>
          <Button variant="phoenix-secondary" className="ms-auto" onClick={handleClose} size="sm">
        ...
        <Modal.Body className="px-5 py-0 mb-2">
          <Form.Group className="mb-4">
            ...
            <Form.Control type="text" placeholder="Enter column name" value={formData.title} name="title" onChange={onChange} required/>
          </Form.Group>
          <Form.Group>
            <Form.Label className="form-label-header mb-2">
              Expected Revenue
            ...
            <Form.Control type="number" placeholder="$ Enter amount" className="flex-1 input-spin-none" value={formData.revenue} name="revenue" required onChange={onChange}/>
          </Form.Group>
        </Modal.Body>
        <Modal.Footer className="d-flex justify-content-end align-items-center border-0 p-5">
          <Button variant="link" className="px-4 m-0 text-danger" onClick={handleClose}>
            Cencel
          </Button>
