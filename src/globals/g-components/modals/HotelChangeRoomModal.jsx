...
import { faBaby, faBath, faBed, faCheck, faCircle, faTimes, faUser } from '@fortawesome/free-solid-svg-icons';
...
const HotelChangeRoomModal = ({ show, handleClose, roomTypes }) => {
    return (<Modal show={show} onHide={handleClose} centered size="xl">
      <Modal.Header className="p-4 justify-content-between">
        ...
        <Button variant="phoenix-secondary" className="px-3" onClick={handleClose}>
          <FontAwesomeIcon icon={faTimes} transform="down-2"/>
        </Button>
      </Modal.Header>
      <Modal.Body className="p-4">
        {roomTypes.map((room, index) => (<Fragment key={room.id}>
            <Row className="g-3 mb-4">
              <Col lg={8} xxl={7}>
                <div className="d-flex align-items-center mb-2">
                  <h4 className="mb-0 fw-semibold pe-3">
                    <FontAwesomeIcon icon={faCircle} className="fs-9 text-body-quaternary me-2" transform="up-2"/>
                    {room.type}
                  </h4>
            ...
            <Row className="mb-4 justify-content-between">
              <Col lg={7}>
                <RoomPictureSlider loop={true} spaceBetween={8} slidesPerView="auto" grabCursor={true}>
                  {room.images.map((img, index) => (<SwiperSlide key={index} className="w-auto">
              ...
              <Col lg={5} xl={4}>
                  ...
                  <Row className="g-3">
                    <Col xs={6}>
                      <h6 className="fw-semibold text-body-secondary">
                        <FontAwesomeIcon icon={faBed} className="text-info me-2"/>
                ...
                minimumIntegerDigits: 2
                    ...
                    <Col xs={6}>
                      <h6 className="fw-semibold text-body-secondary">
                        <FontAwesomeIcon icon={faBaby} className="text-info me-2"/>
                ...
                minimumIntegerDigits: 2
                    ...
                    <Col xs={6}>
                      <h6 className="fw-semibold text-body-secondary">
                        <FontAwesomeIcon icon={faBath} className="text-info me-2"/>
            ...
            <SelectRoomCollapsibleContainer collapseTitle="Room Amenities" id={`amenitiesCollapse-${index}`}>
              <div className="px-md-4 pt-4">
                <h5 className="mb-3">Most popular</h5>
                <Row className="g-0 mb-5">
                  {room.popularAmenities.map(item => (<Col sm={6} lg={4} key={item.id}>
                      <div className={classNames(item.classes, 'd-flex align-items-center gap-2 px-4 py-3 h-100 border-translucent')}>
                        <FontAwesomeIcon icon={item.icon} className="fs-9 text-warning"/>
                ...
                <Row className="g-2">
                  {room.amenities.map((item, index) => (<Col lg={4} xl={3} key={index}>
