...
const ChatSidebar = ({ className }) => {
    ...
    return (<>
      <Card className={classNames(className, 'chat-sidebar p-3 p-xl-1')}>
        <Button className="d-none d-sm-block d-xl-none mb-2" onClick={() => setOpenSearchModal(true)}>
          <FontAwesomeIcon icon={faMagnifyingGlass} className="text-body-tertiary text-opacity-85 fs-7"/>
        </Button>
        <Dropdown className="d-none d-sm-block d-xl-none mb-5" align={isRTL ? 'end' : 'start'}>
          <Dropdown.Toggle variant="" size="sm" className="w-100 mx-auto dropdown-caret-none">
            <FontAwesomeIcon icon={faBars} className="fs-7 text-body-tertiary text-opacity-85"/>
          </Dropdown.Toggle>
        ...
        <Form.Group className="form-icon-container mb-4 d-sm-none d-xl-block">
          <Form.Control type="text" placeholder="People, Groups and Messages" className="form-icon-input"/>
      ...
      <Modal show={openSearchModal} onHide={() => setOpenSearchModal(false)} className="search-box-modal mt-15">
        <Modal.Body className="p-0 bg-transparent">
          <DropdownSearchBox placeholder="Search People, Groups and Messages" size="lg" style={{ width: 'auto' }} autoFocus/>
        </Modal.Body>
      </Modal>
    </>);
};
