...
const ContactsBySource = () => {
    ...
    return (<Row>
      <Col xs={12} sm={7} md={8} xxl={8} className="mb-md-3 mb-lg-0">
        ...
        <Row className="g-0">
          {contactSourceData.map((item, index) => (<Col xs={6} xl={4} key={item.name}>
              <ContactSourceItem value={item.value} label={item.name} color={item.color} className={classNames('border-translucent', {
