...
import LeadEmailsTable, { leadEmailsColumns } from 'components/tables/LeadEmailsTable';
...
const LeadEmails = () => {
    ...
    const tabItems = useMemo(() => {
        const getDataCount = (label) => getPrePaginationRowModel().rows.filter(({ original: { type } }) => type === label).length;
        return [
            {
                label: 'Mails',
                value: 'mails',
    ...
    return (<div>
      ...
      <AdvanceTableProvider {...table}>
        ...
        <SearchBox onChange={handleSearchInputChange} placeholder="Search Emails" className="w-100 mb-3"/>
        <LeadEmailsTable />
