...
import { dealActivities, dealNotes, meetingData, taskList } from 'data/crm/dealDetailsData';
...
import { faChartLine, faClipboard, faEnvelope, faPaperclip, faPhone, faSquareCheck, faVideo } from '@fortawesome/free-solid-svg-icons';
const tabitems = [
    {
        name: 'Activity',
        icon: faChartLine,
...
const DealDetailsTab = () => {
    return (<Tab.Container defaultActiveKey="activity">
      <Nav variant="underline" className="deal-details scrollbar flex-nowrap w-100 pb-1 mb-6">
        {tabitems.map(item => (<Nav.Item key={item.name} className="me-2 text-nowrap">
            <Nav.Link eventKey={item.name.toLowerCase()}>
              <FontAwesomeIcon icon={item.icon} className="me-2 tab-icon-color"/>
              {item.name}
            </Nav.Link>
          </Nav.Item>))}
      </Nav>
      <Tab.Content>
        {tabitems.map(item => (<Tab.Pane key={item.name} eventKey={item.name.toLowerCase()}>
            {item.content}
          </Tab.Pane>))}
      </Tab.Content>
    </Tab.Container>);
