...
const DealColumn = ({ column, handleOpenAddModal }) => {
    ...
    return (<div className="deals-column">
      <div className="d-flex align-items-center justify-content-between position-sticky top-0 z-1 bg-body">
        ...
        <div className="d-flex gap-3">
          ...
          <Dropdown align="end">
            <Dropdown.Toggle variant="" className="dropdown-caret-none p-0 fs-10">
      ...
      <div ref={setNodeRef} {...attributes} {...listeners} className="scrollbar flex-1 d-flex flex-column" onClick={e => e.stopPropagation()}>
        <SortableContext items={column.deals.map(item => item.id)} strategy={verticalListSortingStrategy}>
          {column.deals.map(item => (<DealCard deal={item} key={item.id} columnId={column.id}/>))}
        </SortableContext>
      </div>
    </div>);
};
