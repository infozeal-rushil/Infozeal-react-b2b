import { faHeart, faHome, faShoppingCart, faStar, faUser } from '@fortawesome/free-solid-svg-icons';
...
const ProfileDetailsTab = () => {
    return (<Tab.Container defaultActiveKey="orders">
      <Nav className="mb-3 pb-1 gap-3 nav-underline flex-nowrap scrollbar">
        {tabLinks.map(item => (<Nav.Item key={item.id}>
            <Nav.Link eventKey={item.id} className="text-nowrap">
              <FontAwesomeIcon icon={item.icon} className="me-2"/>
              {item.label}{' '}
              {item.number && (<span className="text-body-tertiary fw-normal">
                  ({item.number})
                </span>)}
            </Nav.Link>
          </Nav.Item>))}
      ...
      <Tab.Content>
        {tabLinks.map(item => (<Tab.Pane key={item.id} eventKey={item.id}>
            {item.content}
          </Tab.Pane>))}
      </Tab.Content>
    </Tab.Container>);
