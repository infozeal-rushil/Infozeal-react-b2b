...
const EmailRow = ({ email, index }) => {
    var _a;
    const { getRowCheckboxProps } = useBulkSelect();
    return (<div className="border-bottom border-translucent hover-actions-trigger py-3">
      <Row className="gx-2">
        <Col xs="auto">
          <div className="d-flex flex-column flex-sm-row">
            <IndeterminateCheckbox {...getRowCheckboxProps(String(index))} className="mb-2 m-sm-0 me-sm-2"/>
        ...
        <Col className="col-auto">
          {email.sender.avatar ? (<Avatar src={email.sender.avatar} size="s"/>) : (<Avatar size="s" variant="name">
              {email.sender.name.charAt(0)}
            </Avatar>)}
        </Col>
        <Col className="col-auto">
          <Link to="#!" className={classNames('fs-9 inbox-link', {
        ...
        <Col className="col-auto ms-auto">
          <div className="hover-actions end-0">
            <Dropdown>
              <Dropdown.Toggle variant="phoenix-secondary" className="dropdown-caret-none btn-icon">
          ...
          <span className={classNames('fs-10', {
      ...
      <div className="ms-4 mt-n3 mt-sm-0 ms-sm-11">
        <Link to="/apps/email/email-detail" className="d-block inbox-link">
          <span className={classNames('fs-9 line-clamp-1', {
        ...
        {(_a = email.attachments) === null || _a === void 0 ? void 0 : _a.map(attachment => (<a href="#!" className="d-inline-flex align-items-center border border-translucent rounded-pill px-3 py-1 me-2 mt-2 inbox-link" key={attachment.id}>
            <FontAwesomeIcon icon={getFileIcon(attachment.format)} className={classNames('fs-9', {
