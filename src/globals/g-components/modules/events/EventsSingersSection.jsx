...
const EventsSingersSection = ({ photos }) => {
    ...
    return (<>
      ...
      <Row className="g-1 g-sm-2 mb-7 mb-xxl-8">
        {photos.map((photo, index) => (<Col key={photo} xs={index === photos.length - 1 ? 6 : 3}>
            <img src={photo} alt="" onClick={() => openLightbox(index + 1)} className="rounded h-100 w-100 fit-cover cursor-pointer"/>
          </Col>))}
      </Row>
