...
const GridGroupView = ({ data }) => {
    ...
    const files = data.filter(file => ['doc', 'zip', 'csc', 'xlx'].includes(file.type));
    const others = data.filter(file => !fileTypes.some(type => file.type.includes(type)));
    const fileGroups = [
        {
            title: 'Folder',
            data: folder
    ...
    return (<>
      {fileGroups.map((fileGroup, index) => fileGroup.data.length > 0 && (<React.Fragment key={fileGroup.title}>
              <h4 className="mb-3">{fileGroup.title}</h4>
              <div className="files-container mb-3">
                {fileGroup.data.map(file => (<FileBox file={file} key={file.id}/>))}
              </div>
