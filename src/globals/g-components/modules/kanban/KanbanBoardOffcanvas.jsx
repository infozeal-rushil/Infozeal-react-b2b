import { faAngleRight, faPencil, faTimes } from '@fortawesome/free-solid-svg-icons';
...
const KanbanBoardOffcanvas = () => {
    ...
    return (<PhoenixOffcanvas open={openBoardDetailsOffcanvas} onHide={handleClose} className="bg-body-highlight" placement="end" fixed style={{ maxWidth: 445 }}>
      ...
      <Offcanvas.Body>
        ...
        <div className="d-flex align-items-center gap-3 mb-5">
          ...
          <div className="flex-1">
            <Link to="#!" className="text-decoration-none text-body-highlight lh-1 fw-semibold">
              Sasha Blaus
            </Link>
        ...
        <div className="d-flex align-items-center gap-2 mb-4">
          {kanbanBoardMembers.members.map(member => (<AvatarDropdown user={member} size="m" key={member.id}/>))}
        </div>

        <h4 className="text-body-highlight fw-semibold mb-3">Guests</h4>
        <div className="d-flex align-items-center gap-2 mb-7">
          {kanbanBoardMembers.guests.map(member => (<AvatarDropdown user={member} size="m" key={member.id}/>))}
        </div>
        ...
        <ul className="list-unstyled mb-0">
          {actions.map(action => (<li key={action.label}>
              <Link to="#!" className={classNames('text-body-highlight fw-semibold text-decoration-none d-flex flex-between-center py-3 border-bottom border-translucent')}>
                <span>{action.label}</span>
                <FontAwesomeIcon icon={faAngleRight} className="fs-9"/>
              </Link>
            </li>))}
          <li>
            <Link to="#!" className={classNames('fw-semibold text-decoration-none d-flex flex-between-center text-danger pt-3')}>
              Leave Board
            </Link>
          </li>
        </ul>
