import { faAngleDown, faBars, faBarsStaggered, faBoxArchive, faCalendarDays, faFilter, faPalette, faRightLeft, faUserPlus } from '@fortawesome/free-solid-svg-icons';
    ...
    return (<>
      <div className="kanban-header">
        <Row className="gx-0 justify-content-between justify-content-md-start">
          <Col xs="auto">
            <Dropdown>
              <Dropdown.Toggle variant="link" className="text-decoration-none dropdown-caret-none text-body-emphasis fs-8 ps-0">
                <span className="fs-7 me-2">Phoenix</span>
                <FontAwesomeIcon icon={faAngleDown} className="text-body-quaternary d-inline-block"/>
              </Dropdown.Toggle>
          ...
          <Col xs="auto" className="d-flex align-items-center gap-4">
            <Avatar.Group total={kanbanBoardMembers.members.length +
            kanbanBoardMembers.guests.length} size="m">
              {kanbanBoardMembers.members.slice(0, 3).map(member => (<AvatarDropdown user={member} size="m" key={member.id}/>))}
            </Avatar.Group>
            <Button startIcon={<FontAwesomeIcon icon={faUserPlus}/>} variant="primary" className="fs-10 px-3" onClick={() => setOpenInviteModal(!openInviteModal)}>
          ...
          <Col md="auto" className="d-flex align-items-center gap-4 ms-auto">
            <Nav className="w-100 fs-9">
              {navItems.map(item => (<Nav.Item key={item.title}>
              ...
              <Nav.Item className="my-auto ms-auto">
                <Nav.Link className="d-flex gap-2 align-items-center text-body px-2 px-xl-3 fw-bold" onClick={() => {
            kanbanDispatch({
      ...
      <KanbanInviteModal show={openInviteModal} handleClose={() => setOpenInviteModal(false)}/>
    </>);
};
