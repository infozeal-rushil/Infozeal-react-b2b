...
const BoardForm = () => {
    const { formData, onChange } = useWizardFormContext();
    return (<>
      ...
      <FloatingLabel label="Board Name" className="mb-4">
        <Form.Control type="text" placeholder="Board Name" name="name" value={formData.name} onChange={onChange}/>
      </FloatingLabel>
      ...
      <FloatingLabel label="Description (Optional)">
        <Form.Control as="textarea" placeholder="Description" style={{ height: 128 }} value={formData.description} name="description" onChange={onChange}/>
      </FloatingLabel>
    </>);
};
