...
import { pricingFeaturesAlternate } from 'data/landing/pricingData';
import React from 'react';
import { Card } from 'react-bootstrap';
const PricingItem = ({ pricing }) => {
    return (<div className="pricing-card">
      <Card className={classNames('bg-transparent', {
            'border border-2 border-info rounded-4': pricing.category === 'Business',
        ...
        <Card.Body className="p-7">
          ...
          <Button variant={pricing.category === 'Business' ? 'primary' : 'outline-primary'} size="lg" className="w-100 mb-7">
            Buy
          </Button>
          <h5 className="mb-4">What’s included</h5>
          <ul className="fa-ul ps-4 m-0 pricing">
            {pricingFeaturesAlternate.map((feature, index) => (<li key={feature.id} className={classNames('d-flex align-items-center', {
                ...
                {feature.new && pricing.category === 'Business' && (<Badge variant="phoenix" bg="info" className="ms-2 fs-10">
                    New
                  </Badge>)}
              </li>))}
          </ul>
