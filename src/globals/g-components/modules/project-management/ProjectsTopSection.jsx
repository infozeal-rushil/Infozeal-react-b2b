...
const ProjectsTopSection = ({ activeView }) => {
    const navigate = useNavigate();
    const { setGlobalFilter, getPrePaginationRowModel, getColumn } = useAdvanceTableContext();
    ...
    return (<Row className="g-3 justify-content-between align-items-center mb-4">
      <Col xs={12} sm="auto">
        <FilterTab tabItems={tabItems}/>
      </Col>
      <Col xs={12} sm="auto">
        <div className="d-flex align-items-center gap-1">
          <SearchBox onChange={handleSearchInputChange} placeholder="Search projects" style={{ maxWidth: '30rem' }} className="me-3"/>
          <ToggleViewButton tooltip="List view" active={activeView === 'list'} onClick={() => {
            navigate('/apps/project-management/project-list-view');
        }}>
            <FontAwesomeIcon icon={faList} className="fs-10"/>
          </ToggleViewButton>
          <ToggleViewButton tooltip="Board view" active={activeView === 'board'} onClick={() => {
            navigate('/apps/project-management/project-board-view');
        }}>
            <NineGrid />
          </ToggleViewButton>
          <ToggleViewButton tooltip="Card view" active={activeView === 'card'} onClick={() => {
