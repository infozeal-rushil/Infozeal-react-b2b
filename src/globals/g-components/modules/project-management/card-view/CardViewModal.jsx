...
const CardViewModal = ({ handleClose, show, project }) => {
    ...
    return (<Modal show={show} onHide={handleClose} size="lg" className="p-0">
        ...
        <Row className="g-5">
          <Col xs={12} xl={9}>
            ...
            <div className="d-flex align-items-center mb-4">
              ...
              <ProgressBar now={progress} className={classNames('flex-1', bgClassName)} variant={variant}/>
            </div>
            <h6 className="text-body-secondary mb-2">Due date</h6>
            <div className="mb-3">
              <div className="w-50">
                <DatePicker placeholder="Set the due date" options={{
            ...
            <div className="mb-3">
              ...
              <div className="d-flex gap-1">
                {project.assigness.slice(0, 5).map(member => (<AvatarDropdown user={member} size="m" key={member.id}/>))}
            ...
            <div className="mb-5">
              ...
              <div className="d-flex gap-2 align-items-center">
                ...
                <Button variant="link" className="p-0 fs-9 text-body fw-bolder text-decoration-none lh-1" startIcon={<FontAwesomeIcon icon={faPlus}/>}>
                  Add another
                </Button>
              </div>
            </div>
            ...
            <div className="bg-body-highlight rounded-2 px-4 mb-3">
              <div className="mb-1">
                {comments.map((comment, index) => (<Comment comment={comment} className={index !== comments.length - 1
                ? 'border-bottom border-translucent'
                : undefined} key={comment.id}/>))}
              </div>
            </div>
            ...
            <div className="mb-7">
              ...
              <div className="mb-4">
                {todoList.map((todo, index) => (<TodoListItem key={todo.task} todo={todo} className={classNames('py-3', {
                'border-top border-translucent': index === 0
            })}/>))}
              </div>
              <Button startIcon={<FontAwesomeIcon icon={faPlus}/>} variant="link" className="text-decoration-none p-0">
                Add new task
              </Button>
            </div>
            ...
            <div>
              ...
              <div className="mb-3">
                {attachments.map((attachment, index) => (<FileListItem key={attachment.name} attachment={attachment} className={classNames({
                'border-top': index === 0
            })}/>))}
              </div>
              <div className="">
                <Button variant="link" className="text-decoration-none p-0" startIcon={<FontAwesomeIcon icon={faPlus} className="me-1"/>}>
                  Add file(s)
                </Button>
              </div>
            </div>
          ...
          <Col xs={12} xl={3}>
            ...
            <div className="mb-6 d-flex flex-column gap-2">
              {addToCardItems.map(item => (<Button key={item.label} variant="subtle-secondary" startIcon={<FontAwesomeIcon icon={item.icon} className="me-2"/>} className="w-100 text-start" size="sm">
                  {item.label}
                </Button>))}
            </div>
            <h5 className="text-body-secondary mb-3">Actions</h5>
            <div className="d-flex flex-column gap-2">
              {actionItems.map(item => (<Button variant="subtle-secondary" startIcon={<FontAwesomeIcon icon={item.icon} className="me-2"/>} className="w-100 text-start" size="sm" key={item.label}>
                  {item.label}
                </Button>))}
            </div>
          </Col>
