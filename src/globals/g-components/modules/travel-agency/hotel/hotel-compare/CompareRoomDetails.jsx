...
import { roomTypes } from 'data/travel-agency/customer/hotelCompare';
import Button from 'components/base/Button';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faBed, faUser, faBaby, faBath } from '@fortawesome/free-solid-svg-icons';
...
const CompareRoomDetails = ({ hotelInfo, reviewFields }) => {
    ...
    return (<>
      <table className="table table-layout-fixed table-compare mb-0">
        ...
        <tbody>
          ...
          <tr>
            ...
            {hotelInfo.map((info, index) => info.rooms.map(item => (<td className={classNames('border-translucent px-3', {
                ...
                'border-end': index !== hotelInfo.length - 1
                  ...
                  <div className="d-flex flex-wrap flex-between-center gap-2">
                    ...
                    <Button variant="phoenix-primary" onClick={() => setShowChangeRoomModal(true)}>
                      Change Room
                    </Button>
                  </div>
                </td>)))}
          ...
          <tr>
            ...
            {hotelInfo.map((info, index) => info.rooms.map(item => (<td className={classNames('border-translucent px-3', {
                ...
                minWidth: 250
                  ...
                  <RoomPictureSlider slidesPerView={3} spaceBetween={8} className="hotel-compare-slider">
          ...
          <tr>
            ...
            {hotelInfo.map((info, index) => info.rooms.map(item => (<td key={item.id} className={classNames('border-translucent px-3', {
                'border-end-lg': index === hotelInfo.length - 1,
                'border-end': index !== hotelInfo.length - 1
            })}>
          ...
          {reviewFields.map(review => (<HotelCompareAmenityRow key={review.id} title={review.title} reviewField={hotelInfo.map(hotel => hotel.popularAmenities[review.field])}/>))}
          <tr>
            <td className="border-0 pb-0"></td>
            {hotelInfo.map(info => (<td className="border-0 px-3 pb-0" key={info.id}>
      ...
      <HotelChangeRoomModal show={showChangeRoomModal} handleClose={() => setShowChangeRoomModal(false)} roomTypes={roomTypes}/>
    </>);
};
