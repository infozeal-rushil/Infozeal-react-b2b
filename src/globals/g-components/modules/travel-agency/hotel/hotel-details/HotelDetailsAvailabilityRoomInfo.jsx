...
import { faBed, faCheck, faCircle, faMugSaucer, faUser } from '@fortawesome/free-solid-svg-icons';
...
const renderIcons = (count, iconClass) => {
    return Array.from({ length: count }).map((_, index) => (<FontAwesomeIcon icon={iconClass} key={index} className={classNames('text-primary fs-9', {
            'me-1': index !== count - 1
        })}/>));
};
const HotelDetailsAvailabilityRoomInfo = ({ room }) => {
    return (<>
          ...
          <Row className="flex-lg-nowrap g-3 mb-2">
            <Col md="auto">
              <h4 className="mb-0 fw-semibold">
                <FontAwesomeIcon icon={faCircle} className="fs-9 text-body-quaternary me-2" transform="up-1"/>
                {room.name}
              </h4>
            </Col>
            <Col md="auto" className="d-flex align-items-center">
              ...
              <div className="vr bg-body-secondary mx-3"/>
              {room.breakfast && (<>
                  <FontAwesomeIcon icon={faMugSaucer} className="text-primary fs-9"/>
                  <div className="vr bg-body-secondary mx-3"/>
                </>)}
          ...
          <Card className="bg-body-highlight">
            <Card.Body>
              <ul className="mb-2 list-unstyled d-flex flex-wrap gap-2 text-body-highlight fs-9 lh-1">
                {room.amenities.slice(0, 6).map((item, index) => (<li key={index} className="me-1 mb-0">
