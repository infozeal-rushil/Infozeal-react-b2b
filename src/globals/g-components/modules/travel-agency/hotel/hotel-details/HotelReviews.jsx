...
import { faBed, faCalendar, faReply, faThumbsDown, faThumbsUp, faUser } from '@fortawesome/free-solid-svg-icons';
import Button from 'components/base/Button';
import classNames from 'classnames';
import { numberFormat } from 'helpers/utils';
const HotelReviews = ({ ratings, reviews }) => {
    return (<>
      <h3 className="mb-5">Reviews</h3>
      <Row className="gx-md-6 gx-xl-8 gy-2">
        {ratings.map((item, index) => (<Col key={index} md={6} lg={5}>
            <Row className="align-items-center g-0">
              ...
              <Col xs={8}>
                ...
                maximumFractionDigits: 1
                  ...
                  <ProgressBar now={parseFloat(item.rating.toString()) * 20} className="w-100"/>
                </div>
              </Col>
            </Row>
          </Col>))}
      ...
      {reviews.map((review, index) => (<Fragment key={index}>
          <div className="d-flex align-items-center position-relative gap-2 mb-3">
            <Avatar src={review.avatar} size="s"/>
            <Link to="#!" className="fw-semibold text-body-emphasis stretched-link">
              {review.name}
            </Link>
          ...
          <div className="d-flex gap-3 mb-5">
            <FontAwesomeIcon icon={faThumbsUp} className="text-success" transform="down-5"/>
            <p className="mb-0">{review.liked}</p>
          </div>
          <div className="d-flex gap-3 mb-5">
            <FontAwesomeIcon icon={faThumbsDown} className="text-body-quaternary" transform="down-5"/>
