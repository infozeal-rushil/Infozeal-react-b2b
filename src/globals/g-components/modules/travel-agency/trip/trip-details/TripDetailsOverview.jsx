import { faClock, faHeart, faLocationDot, faUser } from '@fortawesome/free-solid-svg-icons';
...
const TripDetailsOverview = ({ tripOverview }) => {
    ...
    return (<>
      <div className="d-flex flex-wrap align-items-center gap-3">
        ...
        <div className="me-3">
          <GenerateStar filledStars={tripOverview.totalRate} className="me-1 fs-9"/>
        ...
        <h5 className="fw-semibold text-body-tertiary me-3">
          <span className="p-2 d-inline-flex bg-danger-subtle rounded-pill me-2">
            <FontAwesomeIcon icon={faHeart} className="fs-9 text-danger-light" transform="down-1"/>
          </span>
          Recommended by {numberFormat(tripOverview.recommendBy, 'standard')}{' '}
          travellers
        </h5>
        <Button variant="primary" className="ms-md-auto" onClick={() => setModalShow(true)}>
          Check availability
        </Button>
      </div>
        ...
        <Row className="gy-4">
          <Col md={4} lg={3} className="border-end-md">
            ...
            <div className="d-flex flex-wrap">
              {tripOverview.tripTags.map(tag => (<Link key={tag} to="#!" className="me-2">
      ...
      <TripDetailsAvailabilityModal show={modalShow} onHide={() => setModalShow(false)}/>
    </>);
};
