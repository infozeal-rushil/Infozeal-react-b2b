...
const collapsibleContainerContent = (tripDetailsItems) => {
    return [
        {
            id: 'collapseWhatToExpect',
            title: 'What to expect',
            defaultOpen: true,
            content: (<TripDetailsTabDetailsExpectation expectations={tripDetailsItems.expectation}/>)
        },
        {
            id: 'collapseIncluded',
            ...
            defaultOpen: false,
            className: 'mt-4',
            content: (<TripDetailsTabDetailsIncluded includedItems={tripDetailsItems.includeOrExclude}/>)
        },
        {
            ...
            defaultOpen: false,
            className: 'mt-4',
            content: (<TripDetailsTabDetailsDeparture departureItems={tripDetailsItems.departOrReturn}/>)
        },
        {
            ...
            defaultOpen: false,
            className: 'mt-4',
            content: (<TripDetailsTabDetailsAccessibility accessibility={tripDetailsItems.accessibility}/>)
        },
        {
            ...
            className: 'mt-4',
            defaultOpen: false,
            content: (<div className="py-6 px-4">
          <ul className="px-0">
            {tripDetailsItems.additional.map(item => (<li key={item.id} className="mb-1 d-flex">
        ...
        },
        {
            ...
            defaultOpen: false,
            className: 'mt-4',
            content: (<TripDetailsTabDetailsPolicy policies={tripDetailsItems.policy}/>)
        }
    ];
};
const TripDetailsTabDetailsContent = ({ tripDetailsItems }) => {
    const contents = useMemo(() => {
        return collapsibleContainerContent(tripDetailsItems);
    }, [tripDetailsItems]);
    return (<Row className="justify-content-between gx-0 gy-5">
      <Col xl={7}>
        {contents.map(item => (<CollapsibleContainer key={item.id} id={item.id} collapseTitle={item.title} titleClass="fs-8 fs-sm-7 text-body-highlight" className={item.className} containerSize="trip" defaultOpen={item.defaultOpen}>
            {item.content}
