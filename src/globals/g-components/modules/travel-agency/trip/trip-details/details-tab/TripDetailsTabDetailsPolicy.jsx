...
const TripDetailsTabDetailsPolicy = ({ policies }) => {
    return (<div className="py-6 px-4">
      {policies.map((policy, index) => (<Card key={policy.id} className={classNames('bg-transparent', {
                'mb-3': index !== policies.length - 1
            })}>
          <Card.Body>
            <h5 className="mb-3">{policy.label}</h5>
            <ul className="list-unstyled mb-0">
              {policy.policyItems.map(item => (<li key={item.id} className="mb-3 d-flex">
