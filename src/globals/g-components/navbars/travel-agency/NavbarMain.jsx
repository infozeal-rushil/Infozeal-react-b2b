...
const NavDropdownItems = ({ label, items, isActive, pathName }) => {
    ...
    return (<Dropdown as="li" className="nav-item dropdown-hoverable" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} autoClose="outside" onToggle={() => setShow(!show)} show={show}>
      <Dropdown.Toggle as={Link} className={classNames('nav-link fs-8 fw-bold', {
            active: isActive
        })} to="#!">
        {label}
      </Dropdown.Toggle>
      <Dropdown.Menu className="navbar-dropdown-caret mt-lg-3 mb-3 mb-lg-0">
        {items.map(dropdownItem => (<Dropdown.Item as={Link} to={dropdownItem.url} key={dropdownItem.id} className={classNames({ active: pathName === dropdownItem.url })}>
            {dropdownItem.label}
          </Dropdown.Item>))}
      </Dropdown.Menu>
    </Dropdown>);
...
const NavbarMain = () => {
    ...
    useEffect(() => {
        const toggleShadowClass = () => {
            ...
            if (window.scrollY > 300) {
                (_a = containerRef.current) === null || _a === void 0 ? void 0 : _a.classList.add('navbar-shadow');
            }
            else {
                (_b = containerRef.current) === null || _b === void 0 ? void 0 : _b.classList.remove('navbar-shadow');
            }
        };
    ...
    return (<div className="bg-body sticky-top" ref={containerRef}>
      <Navbar expand="lg" className="navbar-landing container-medium border-0 px-3 py-2" expanded={isNavItemOpen} onToggle={() => setNavItemsOpen(!isNavItemOpen)}>
        <Navbar.Toggle className="fs-8 ps-2 me-sm-2 border-0 ms-n2 hover-bg-transparent navbar-toggler-humburger-icon">
          <span className="navbar-toggle-icon">
            <span className="toggle-line"/>
          </span>
        </Navbar.Toggle>
        <Navbar.Brand as={Link} to="/" className="flex-1 flex-lg-grow-0 me-lg-8 me-xl-13">
          <Logo textClass="d-none d-sm-block"/>
        </Navbar.Brand>
        <div className="d-flex align-items-center gap-3 gap-md-4 my-2 order-lg-1">
          <ThemeToggler />
          <Button to="#!" as={Link} variant="link" className="text-body-tertiary p-0">
        ...
        <Navbar.Collapse id="navbarSupportedContent">
          <Nav as="ul" className="me-auto travel-nav-top">
            {navItems.map(item => item.items ? (<NavDropdownItems key={item.id} label={item.label} items={item.items} isActive={pathNameList.includes(item.label.toLowerCase())} pathName={pathname}/>) : (<Nav.Item as="li" key={item.id} className="">
                  <Nav.Link as={Link} to="#!" className={classNames('fw-bold', {
