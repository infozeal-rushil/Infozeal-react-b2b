import { UilEstate, UilGift, UilLamp, UilMobileAndroid, UilMonitor, UilPalette, UilPlaneDeparture, UilShoppingBag, UilStar, UilWatchAlt, UilWrench } from '@iconscout/react-unicons';
...
const EcomCategoryNavs = () => {
    return (<div className="d-flex justify-content-between">
      {categories.map(category => (<EcomCategoryNavItem key={category.label} category={category}/>))}
    </div>);
};
const EcomCategoryNavItem = ({ category }) => {
    return (<Link to={category.url} className="icon-nav-item mb-3">
