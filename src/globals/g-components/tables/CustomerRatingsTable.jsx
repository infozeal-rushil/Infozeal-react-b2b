...
import { customerRatingsTableData } from 'data/e-commerce/customers';
const columns = [
    {
        accessorKey: 'product',
        header: () => 'Product',
        cell: ({ row: { original } }) => {
            ...
            return (<Link to="#!" className="fw-semibold line-clamp-1">
          {product}
        </Link>);
        },
        enableSorting: true,
    ...
    },
    {
        ...
        cell: ({ row: { original } }) => {
            ...
            return (<p className="fw-semibold text-body-highlight mb-0 line-clamp-2">
          {review.slice(0, 134)}
          {review.length > 134 && (<>
              {`...`}
    ...
    },
    {
        ...
        header: 'STATUS',
        cell: ({ row: { original } }) => {
            const { status: { title, badgeBg, icon } } = original;
            return (<Badge bg={badgeBg} variant="phoenix" iconPosition="end" className="fs-10" icon={<FeatherIcon icon={icon} size={12} className="ms-1"/>}>
          {title}
        </Badge>);
        },
        meta: {
    ...
    return (<>
      <AdvanceTableProvider {...table}>
        <div className="border-y border-translucent">
          <AdvanceTable tableProps={{ className: 'phoenix-table fs-9 mb-0' }} rowClassName="hover-actions-trigger btn-reveal-trigger position-static"/>
