...
import { dealForecastTableData } from 'data/crm/dashboardData';
...
const columns = [
    {
        ...
        cell: ({ row: { original } }) => {
            ...
            return (<Link to={contact.profileLink} className="fw-semibold">
          {contact.name}
        </Link>);
        },
        meta: {
    ...
    {
        id: 'appointment',
        header: () => (<div className="d-inline-flex flex-center gap-2">
        ...
        footer: ({ table }) => numberFormat(table
            .getFilteredRowModel()
            .rows.reduce((total, row) => total + row.getValue('appointment'), 0))
    },
    {
        id: 'qualified',
        header: () => (<div className="d-inline-flex flex-center gap-2">
        ...
        footer: ({ table }) => currencyFormat(table
            .getFilteredRowModel()
            .rows.reduce((total, row) => total + row.getValue('qualified'), 0))
    },
    {
        id: 'closed_won',
        header: () => (<div className="d-inline-flex flex-center gap-2">
        ...
        footer: ({ table }) => currencyFormat(table
            .getFilteredRowModel()
            .rows.reduce((total, row) => total + row.getValue('closed_won'), 0))
    },
    {
        id: 'contact_sent',
        header: () => (<div className="d-inline-flex flex-center gap-2">
        ...
        footer: ({ table }) => currencyFormat(table
            .getFilteredRowModel()
            .rows.reduce((total, row) => total + row.getValue('contact_sent'), 0))
    }
];
const DealForecastTable = () => {
    ...
    return (<AdvanceTableProvider {...table}>
      <AdvanceTable hasFooter tableProps={{
            size: 'sm',
