...
const EcomCartTable = ({ products }) => {
    return (<Scrollbar autoHeight autoHeightMax="100%" className="table-scrollbar">
      <Table className="phoenix-table fs-9 mb-0 border-top border-translucent">
        ...
        <tbody className="list" id="cart-table-body">
          {products.map(product => (<EcomCartTableRow product={product} key={product.id}/>))}

          <tr className="cart-table-row">
            <td className="text-body-emphasis fw-semibold ps-0 fs-8" colSpan={6}>
              Items subtotal :
            </td>
...
const EcomCartTableRow = ({ product }) => {
    const [quantity, setQuantity] = useState(product.quantity);
    const total = useMemo(() => {
        return product.price * quantity;
    }, [quantity]);
    return (<tr className="cart-table-row" key={product.id}>
      <td className="py-0">
      ...
      <td className="fs-8 ps-5">
        <QuantityButtons type="secondary" quantity={quantity} setQuantity={setQuantity}/>
      </td>
      <td className="fw-bold text-body-highlight text-end">
        {currencyFormat(total)}
      </td>
      <td className="text-end ps-3">
        <Button size="sm" variant="link" className="text-body-quaternary text-body-tertiary-hover me-2">
