...
import { customerOrders } from 'data/e-commerce';
...
const columns = [
    {
        accessorKey: 'orderId',
        header: 'Order',
        cell: ({ row: { original } }) => (<Link to="#!" className="fw-semibold text-primary">
        {original.orderId}
      </Link>),
        meta: {
    ...
    },
    {
        ...
        header: 'Status',
        cell: ({ row: { original } }) => (<Badge bg={original.payment_status.type} variant="phoenix" iconPosition="end" className="fs-10" icon={<FeatherIcon icon={original.payment_status.icon} size={12} className="ms-1"/>}>
