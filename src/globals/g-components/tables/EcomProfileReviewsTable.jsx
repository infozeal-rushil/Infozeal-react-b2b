...
import { latestReviewsTableData } from 'data/LatestReviewsTableData';
...
const columns = [
    {
        ...
        cell: ({ row: { original } }) => {
            ...
            return (<Link to="#!" className="fw-semibold line-clamp-1">{`${product.slice(0, 46)}${product.length > 46 ? '...' : ''}`}</Link>);
        },
        meta: {
    ...
    },
    {
        ...
        cell: ({ row: { original } }) => {
            ...
            return (<p className="fs--1 fw-semibold text-body-highlight mb-0 line-clamp-3">
          {review.slice(0, 134)}
          {review.length > 134 && (<>
              {`...`}
