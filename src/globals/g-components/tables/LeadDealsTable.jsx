...
const columns = [
    {
        header: 'deal name',
        accessorKey: 'name',
        cell: ({ row: { original } }) => (<Link to="#!" className="fw-semibold">
        {original.name}
      </Link>),
        meta: {
    ...
    },
    {
        accessorKey: 'stage',
        header: 'Stage',
        cell: ({ row: { original } }) => (<Badge variant="phoenix" bg={getBadgeBg(original.stage)}>
        {original.stage}
      </Badge>),
        meta: {
    ...
    },
    {
        ...
        cell: ({ row: { original } }) => {
          ...
          <ProgressBar now={probability} style={{ height: 3 }} variant={variant} className="bg-primary-subtle"/>
        </>);
        },
        meta: {
    ...
    },
    {
        accessorKey: 'type',
        header: () => 'Type',
        cell: ({ row: { original } }) => (<Badge variant="phoenix" bg={getBadgeBg(original.type)}>
        {original.type}
      </Badge>),
        meta: {
            headerProps: {
