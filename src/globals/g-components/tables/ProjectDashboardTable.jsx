...
const columns = [
    {
        ...
        cell: ({ row: { original } }) => {
            const { name } = original;
            return (<Link to="/apps/project-management/project-details" className="text-decoration-none fw-bold fs-8">
          {name}
        </Link>);
        },
        meta: {
    ...
    },
    {
        ...
        cell: ({ row: { original } }) => {
            ...
            return (<Avatar.Group total={assigness.length} size="s" className="overflow-hidden">
          {assigness.slice(0, 4).map(assigne => (<AvatarDropdown user={assigne} size="s" key={assigne.id}/>))}
        </Avatar.Group>);
        },
        meta: {
    ...
    },
    {
        ...
        cell: ({ row: { original } }) => {
            ...
            <p className="fw-bold text-body-emphasis fs-9 mb-0">
              {calculation === null || calculation === void 0 ? void 0 : calculation.amount}
            </p>
            <p className="fw-semibold fs-10 text-body-tertiary mb-0">
              {calculation === null || calculation === void 0 ? void 0 : calculation.label}
            </p>
          </>);
            }
            return (<Button variant="phoenix-secondary" className="btn-icon">
    ...
    },
    {
        ...
        cell: ({ row: { original } }) => {
          ...
          <ProgressBar now={(progress.min / progress.max) * 100} style={{ height: 3 }} variant="success"/>
        </>);
        },
        meta: {
            cellProps: { className: 'ps-3' },
    ...
    },
    {
        ...
        cell: ({ row: { original } }) => {
            ...
            return (<ProgressBar style={{ height: 3 }} className="progress-stack">
          {Object.keys(statusProgress).map(item => (<OverlayTrigger key={item} overlay={<Tooltip id={`tooltip-${item}`}>
                  {item === 'ongoing' && 'Active'}
              ...
              <ProgressBar variant={classNames({
                    ...
                    })} now={statusProgress[item]} isChild/>
            </OverlayTrigger>))}
        </ProgressBar>);
        },
        meta: {
