...
const EmailLayout = ({ children, page }) => {
    ...
    return (<div className="email-container">
      <Row className="gx-lg-4 gx-xl-6 gx-3 py-4 z-2 position-sticky bg-body email-header">
        <Col className="col-auto">
          <Button variant="primary" className="email-sidebar-width d-none d-lg-block" as={Link} to="/apps/email/compose">
            Compose
          </Button>
          <Button variant="phoenix-secondary" className="px-3 text-body-tertiary d-lg-none" onClick={() => setOpenSidebar(true)}>
            <FontAwesomeIcon icon={faBars}/>
          </Button>
        </Col>
        {page !== 'compose' && (<Col className="col-auto d-lg-none">
      ...
      <Row className="g-lg-4 g-xl-6 mb-8">
        <Col lg="auto">
          <div className="email-sidebar email-sidebar-width bg-body d-none d-lg-block" id="emailSidebarColumn">
            <EmailSidebar />
          </div>
          {breakpoints.down('lg') && (<PhoenixOffcanvas open={openSidebar} fixed placement="start" className="email-sidebar email-sidebar-width bg-body" backdropClassName="top-0" onHide={() => setOpenSidebar(false)}>
              <EmailSidebar hideSidebar={() => setOpenSidebar(false)}/>
            </PhoenixOffcanvas>)}
        </Col>
        {page !== 'inbox' && (<Col xs="3" className="d-none d-xxl-block">
            <div className="email-content scrollbar">
              <div className="px-lg-1">
                <BulkSelectProvider data={emails}>
                  <InboxToolbar size="sm"/>
                  {emails.map((email, index) => (<EmailRow email={email} index={index} key={email.id}/>))}
                </BulkSelectProvider>
              </div>
            </div>
          </Col>)}
